...
CSkeletonBasics::CSkeletonBasics() :
	...
	m_hNextImageEvent(INVALID_HANDLE_VALUE),
	m_pSkeletonStreamHandle(INVALID_HANDLE_VALUE),
	m_pImageStreamHandle(INVALID_HANDLE_VALUE),
	m_bSeatedMode(false),
...
CSkeletonBasics::~CSkeletonBasics()
{
	...
	if (m_hNextImageEvent && (m_hNextImageEvent != INVALID_HANDLE_VALUE))
	{
		CloseHandle(m_hNextImageEvent);
	}

	if (m_pImageStreamHandle && (m_pImageStreamHandle != INVALID_HANDLE_VALUE))
	{
		//CloseHandle(m_pImageStreamHandle);
	}
...
int CSkeletonBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	//fout = fopen_s("dataout.txt", "w");
	fopen_s(&fout, skeleton_out, "w");
...
void CSkeletonBasics::Update()
{
	...
	if (WAIT_OBJECT_0 == WaitForSingleObject(m_hNextSkeletonEvent, 0) && WAIT_OBJECT_0 == WaitForSingleObject(m_hNextImageEvent, 0))
    {
        ProcessSkeleton();
    }
}
...
HRESULT CSkeletonBasics::CreateFirstConnected()
{
    ...
    if (NULL != m_pNuiSensor)
    {
        ...
        if (SUCCEEDED(hr))
        {
			...
			m_hNextImageEvent = CreateEventW(NULL, TRUE, FALSE, NULL);
...
void CSkeletonBasics::ProcessSkeleton()
{
	...
	hr = m_pNuiSensor->NuiImageStreamGetNextFrame(m_pImageStreamHandle, 0, &imageFrame);
	if (FAILED(hr))
	{
		return;
	}
	...
	cv::Mat frame = cv::Mat(cColorHeight, cColorWidth, CV_8UC4, image);
	outputVideo << frame;
	...
	hr = m_pBitmap->CopyFromMemory(NULL, image, 4 * cColorWidth);
	m_pRenderTarget->DrawBitmap(m_pBitmap, D2D1_RECT_F{ (float)width, (float)height });
	for (int i = 0; i < NUI_SKELETON_COUNT; ++i)
	{
		NUI_SKELETON_TRACKING_STATE trackingState = skeletonFrame.SkeletonData[i].eTrackingState;
...
HRESULT CSkeletonBasics::EnsureDirect2DResources()
{
    ...
    if (NULL == m_pRenderTarget)
    {
		...
		hr = m_pRenderTarget->CreateBitmap(
			D2D1_SIZE_U{ cColorWidth, cColorHeight },
