...
public:
	...
	inline void drawResult(Mat& frame,double scale)
	{
		...
		Rect scaledWin = scaleWin(win,1/TRACKING_TO_BODYSIZE_RATIO);

		if (!getIsNovice()) {
			vector<Point>::iterator it;
			rectangle(frame, scaledWin,COLOR(_ID),2);
	...
	inline int getTemplateNum(){return (int)_template_list.size();}
...
private:
	...
	{
		_kf.statePost=*(Mat_<float>(6,1)<<win.x+0.5*win.width,win.y+0.5*win.height,0,0,0,0);
	}
